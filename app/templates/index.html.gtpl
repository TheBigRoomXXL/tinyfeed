...
<head>
	...
	{{range .Stylesheets}}
	<link rel="stylesheet" type="text/css" href="{{.}}" nonce="{{$.Metadata.nonce}}">
	{{end}}

	{{range .Scripts}}
	<script type="module" src="{{.}}" nonce="{{$.Metadata.nonce}}"></script>
	{{end}}
</head>
...
<body>
	...
	<footer>
		<p>Last Updated On:
			<time id="update-at" datetime="{{.Metadata.datetimeRFC3339}}">
				{{.Metadata.day}}, {{.Metadata.datetime}}
			</time>
		</p>
...
<script type="module" nonce="{{$.Metadata.nonce}}">
